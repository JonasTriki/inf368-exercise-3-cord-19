...
def print_html(df: pd.DataFrame) -> None:
    '''
    Displays pd.DataFrame object as HTML 
    '''
    try:
        display(HTML(df.to_html()))
    except:
        # If it fails, just do normal print
        print(df)

def argsort_regex(pattern: str, x: Iterable) -> np.ndarray:
    '''
    Uses re.findall and then argsorts results
    Regex pattern should have exactly have one group
    Returns argsort indices
    '''
    return np.argsort(re.findall(pattern, str(list(x))))

def sort_regex(pattern: str, x: Iterable) -> np.ndarray:
    '''
    Uses re.findall and then sorts results
    Regex pattern should have exactly have one group
    Returns np.ndarray of sorted items
    '''
    return np.array(x)[argsort_regex(pattern, x)]

def interleave(A: Iterable, B: Iterable) -> tuple:
    '''
